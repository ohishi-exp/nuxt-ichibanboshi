 ...
 * 運賃リスト / 得意先ネット / 品名グルーピング / 得意先前年比 / 再計算) への
 * 横断リンクを常時表示し、どのページからでも他の主要ページへ 1 クリックで
 * 移動できるようにする (得意先ネット・品名グルーピングは当初 `/unchin` ページ
 * 固有の extra slot リンクだったが、user 2026-07-01「得意先ネットもいれて」
 * 「品名グルーピングもいれて」で共通ナビに昇格した)。
 *
 * `検証` (旧 `/admin/verify`) は独立ナビ項目としては無い。再計算ページと内容が
 * ほぼ重複していた (user 2026-07-01「同じ内容含むならいらないじゃん」) ため
 * `/admin/recalc` に検証パネルを統合し、`/admin/verify` ページ自体を削除した。
 *
...
const NAV_LINKS: NavLink[] = [
  ...
  { to: '/admin/recalc', label: '再計算 (検証含む)', icon: '🔧' },
]
