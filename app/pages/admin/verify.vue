...
/**
 * PHP vs Rust 担当者別売上 検証ページ (Phase 1、Refs ohishi-exp/rust-ichibanboshi#41)。
 *
 * shell の `scripts/verify_uriage_diff_php_vs_rust.sh` を nuxt UI 化したもの。
 * 単日 × 営業所 × cal の組み合わせを並列 fetch し、進捗バーと NG 行の diff
 * 詳細を表示する。
 *
 * 営業所 id は user 入力 (CSV)。masters に居ない id は 404 で skip 扱い。
 */
...
const counts = computed(() => {
  const c = { total: jobs.value.length, done: 0, ok: 0, ng: 0, err: 0, skipped: 0, pending: 0, running: 0 }
...
async function runOne(job: VerifyJob): Promise<void> {
  ...
  try {
    ...
    if (err.statusCode === 404) {
      // office_id が masters に居ない → skip 扱い
      job.status = 'skipped'
...
function stop() {
  // 実行中の Promise は abort できないが、queue を空にすれば worker が抜ける
  // pending job は cancelled として扱う
  for (const j of jobs.value) {
...
function statusBadge(s: JobStatus): string {
  ...
  if (s === 'ng') return 'text-red-700 bg-red-100'
  if (s === 'err') return 'text-orange-700 bg-orange-100'
  if (s === 'skipped') return 'text-gray-500 bg-gray-100'
  ...
  <div class="min-h-screen bg-gray-100">
    <header class="bg-white shadow">
      <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        <h1 class="text-xl font-bold">PHP vs Rust 検証</h1>
    ...
    <main class="max-w-7xl mx-auto px-4 py-6">
      ...
      <div v-if="counts.total > 0" class="bg-white rounded-lg shadow p-4 mb-4 space-y-3">
        <div class="text-sm">
      ...
      <div v-if="ngJobs.length > 0" class="bg-white rounded-lg shadow p-4 mb-4">
      ...
      <div v-if="errJobs.length > 0" class="bg-white rounded-lg shadow p-4 mb-4">
      ...
      <div v-if="jobs.length > 0 && counts.ng === 0 && counts.err === 0 && counts.done < counts.total" class="text-xs text-gray-500 mt-2">
        進行中… (途中までは ✅ 全件一致)
      </div>
    </main>
  </div>
